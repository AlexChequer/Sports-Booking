...
async def create_booking(court_id: int, slot_id: int, extras: list[str] | None = None, notes: str | None = None, payload=Depends(verify_token)):
    estimate = calculate_quote(court_id, slot_id, extras or [])
    lock = agenda_client.create_lock(court_id=court_id, slot_id=slot_id, booking_id=0)
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
        """
...
async def list_bookings(payload=Depends(verify_token)):
    conn = get_conn()
    cur = conn.cursor()
...
async def checkout_booking(booking_id: int, method: str, coupon: str | None = None, payload=Depends(verify_token)):
    conn = get_conn()
    cur = conn.cursor()
